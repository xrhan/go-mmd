...
func varBytes(tag byte, buff *Buffer) (interface{}, error) {
	...
	return buff.RequireNext(int(sz))
}
...
func decodeMap(buff *Buffer, num uint) (interface{}, error) {
	if num < 0 {
		return nil, fmt.Errorf("Invalid map size: %d", num)
	}
...
func fastInt(sz byte, buff *Buffer) (ret int, err error) {
	...
	case 0x02:
		var v int16
		v, err = buff.ReadInt16()
		ret = int(v)
	case 0x04:
		var v int32
		v, err = buff.ReadInt32()
		ret = int(v)
	case 0x08:
		var v int64
		v, err = buff.ReadInt64()
		ret = int(v)
	}
	return
}
...
func fastUInt(sz byte, buff *Buffer) (ret uint, err error) {
	...
	case 0x02:
		var v uint16
		v, err = buff.ReadUInt16()
		ret = uint(v)
	case 0x04:
		var v uint32
		v, err = buff.ReadUInt32()
		ret = uint(v)
	case 0x08:
		var v uint64
		v, err = buff.ReadUInt64()
		ret = uint(v)
	}
	return
}
...
func decodeClose(tag byte, buff *Buffer) (interface{}, error) {
	chanID, err := buff.RequireNext(16)
	if err != nil {
		return nil, err
	}
	body, err := Decode(buff)
...
func decodeMessage(tag byte, buff *Buffer) (ret interface{}, err error) {
	if chanID, err := buff.RequireNext(16); err == nil {
		if body, err := Decode(buff); err == nil {
			ret = ChannelMsg{
				Channel: ChannelId(chanID),
				Body:    body,
